...
export default {
  ...
  methods: {
    async UserLogin() {
      ...
      try {
        const response = await axios.get(`https://nemsu-grading.online/api/students?filters[username][$eq]=${this.username}&filters[password][$eq]=${this.password}&filters[activated][$eq]=true`);
