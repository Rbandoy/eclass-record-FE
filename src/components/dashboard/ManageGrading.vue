  ...
  <div class="container mx-auto p-6">
    ...
    <div class="mb-6">
      ...
      <div class="flex flex-row justify-between mt-10">
        ...
        <div v-if="programExists" class="mt-4">
          <button @click="openAddStudentModal" class="bg-green-500 text-white px-4 py-2 rounded-md">
        ...
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
          ...
          <form @submit.prevent="addStudentToProgram">
            ...
            <div class="flex justify-end">
              <button @click="closeAddStudentModal" type="button" class="bg-gray-500 text-white px-4 py-2 rounded-md mr-2">
                Cancel
              </button>
...
const searchProgram = async () => {
  try {
    ...
    const response = await axios.get(`http://localhost:1337/api/subjects?filters[code][$eq]=${programCode.value}`, {
      headers: {
    ...
    if (response.data.data.length > 0) {
      currentProgramId.value = response.data.data.id;
...
const fetchStudents = async (programCode) => {
  try {
    ...
    const gradingsResponse = await axios.get(`http://localhost:1337/api/gradings?filters[program_code][$eq]=${programCode}&filters[instructor_id][$eq]=${profile.id}`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
    ...
    const studentDetailsRequests = studentIds.map(id =>
      axios.get(`http://localhost:1337/api/students?filters[student_id][$eq]=${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
