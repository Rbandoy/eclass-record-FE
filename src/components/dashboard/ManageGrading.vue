  ...
  const renameFile = async () => { 
    ...
    const fetchFiles = async () => {
      ...
      try {
        const config = {
          ...
          url: 'https://nemsu-grading.online/api/upload/files/'
        };
    ...
    const loadFile = async () => {
      if (selectedFile.value) {
        try {
          ...
          const response = await fetch(`https://nemsu-grading.online${selectedFile.value.url}`);
    ...
    const fetchSchoolYear = async () => {
      let config = {
          method: 'get',
          maxBodyLength: Infinity,
          url: 'https://nemsu-grading.online/api/school-years?filters[active][$eq]=active',
          headers: { 
...
const saveToExcel = async () => {
  ...
  try {
    try {
      await axios.delete(`https://nemsu-grading.online/api/upload/files/${selectedFile.value.id}`)
    } catch (error) {
    ...
    const response = await fetch('https://nemsu-grading.online/api/upload/', {
      method: 'POST',
      body: formData,
...
const submitToAdmin = async () => {
  console.log(data.value)
    ...
    const response = await axios.get(`https://nemsu-grading.online/api/school-years?filters[year][$eq]=${schoolYear}&filters[sem][$eq]=${semester}&filters[active][$eq]=true`, {
      headers: {
        'Authorization': `Bearer ${sessionStorage.getItem('jwt')}`
      }
    });
  ...
  try {
    const token = sessionStorage.getItem("jwt")
    const response = await axios.post('https://nemsu-grading.online/api/grade-masterlists', {data}, {
      headers: {
        'Content-Type': 'application/json',
 ...
 formData2.append('files', blob, selectedFile.value.name+""+extension.value); // Change filename if needed
  ...
  try {
    ...
    await fetch('https://nemsu-grading.online/api/upload/', {
      method: 'POST',
      body: formData2,
      // Uncomment if you need to send authorization token
      // headers: { 'Authorization': `Bearer ${token}` },
