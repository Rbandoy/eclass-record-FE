<template>
  <div class="container mx-auto p-6">
    ...
    <div class="bg-blue-500 h-32 rounded-lg mb-6 relative">
      ...
      <div class="absolute bottom-0 left-0 right-0 flex flex-col items-center pb-4">
        <!-- Profile Picture Section with Clickable Upload -->
        <div class="bg-gray-100 p-2 rounded-full shadow-md border-2 border-gray-300 cursor-pointer" @click="triggerFileInput">
          <img :src="profPicture" alt="Profile Picture" class="w-24 h-24 rounded-full object-cover" />
        ...
        <!-- Conditional Rendering of Upload and Save Buttons -->
          ...
          >
          <svg width="30" height="30" fill="white" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <title>save</title> <path d="M9.467 3.106h3.030c0.266 0 0.482 0.278 0.482 0.621v4.374c0 0.343-0.216 0.621-0.482 0.621h-3.030c-0.266 0-0.482-0.278-0.482-0.621v-4.374c0-0.343 0.216-0.621 0.482-0.621zM21.964 14.524l7.987 7.987h-3.993v7.987h-7.987v-7.987h-3.993l7.987-7.987zM17.422 24.768h-10.309c-0.276 0-0.499-0.223-0.499-0.499v-11.481c0-0.276 0.223-0.499 0.499-0.499h17.721c0.276 0 0.499 0.223 0.499 0.499v4.402l3.12 3.12v-13.262c0-0.276-4.216-4.493-4.493-4.493h-4.741c0.169 0.069 0.287 0.223 0.287 0.404v6.181c0 0.244-0.216 0.442-0.482 0.442h-10.594c-0.266 0-0.482-0.198-0.482-0.442v-6.181c0-0.18 0.118-0.335 0.287-0.404h-4.242c-0.277 0-0.499 0.223-0.499 0.499v23.961c0 0.277 0.223 0.499 0.499 0.499h13.429v-2.746zM26.506 22.903v4.611h1.449c0.277 0 0.499-0.223 0.499-0.499v-4.111l-1.948 0z"></path> </g></svg>
          </p> 
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      profile: {
        ...
        confirmed: false,
        blocked: false,
        profilePicture: 'https://api.nemsu-grading.online/uploads/23_5768fecede.png'
      },
      profPicture: '',
  ...
  methods: {
    ...
    async uploadFile() {
      ...
      try {
      await axios.delete(`https://api.nemsu-grading.online/api/upload/files/${this.profile.profilePicture[0].id}`)
      } catch (error) {
        // 
      }
    ...
    async fetchFiles() { 
      ...
      try {
        ...
        const profilePicture = response.data.reduce((acc, cur) => { 
