...
const loadStudents = async () => {
  try {
    ...
    const response = await axios.get('https://nemsu-grading.online/api/students', {
      headers: {
...
const createStudent = async () => {
  try {
    isLoading.value = true
    const token = sessionStorage.getItem('jwt');
    await axios.post('https://nemsu-grading.online/api/students', { data: studentForm.value }, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
...
const updateStudent = async () => {
  try {
    isLoading.value = true
    const token = sessionStorage.getItem('jwt');
    await axios.put(`https://nemsu-grading.online/api/students/${currentStudentId.value}`, { data: studentForm.value }, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
...
const deleteStudent = async (id) => {
  try {
    isLoading.value = true
    const token = sessionStorage.getItem('jwt');
    await axios.delete(`https://nemsu-grading.online/api/students/${id}`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    });
